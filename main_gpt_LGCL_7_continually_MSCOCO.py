    ...
    def train_one_epoch(self, epoch, task_name, task_num):
        ...
        for image, input_ids, labels, image_path in prog:
            ...
            with autocast():
                ...
                # loss = alpha * loss + beta * loss_lgcls + gamma * loss_nouns  # 1st test
                # loss = loss + loss_lgcls + loss_nouns                         # 2nd test
...
if __name__ == '__main__':
    ...
    if train_config.used_dataset == "MSCOCO":
        ...
        df = pd.read_csv("ms_coco_data_1000_5_fixed.csv")
        ...
        if train_true:
            for task in task_list:
                ...
                if train_config.used_dataset == "cont_cap":
                    ...
                    test_df_train = df[df['split'] == 'test'].copy()
                    test_df_train = df[df['category_name'] == task].copy()
        ...
        if forg_calc:
            for task in range(1): #  for task in range(len(task_list)-1, -1, -1):
                ...
                if train_config.used_dataset == "cont_cap":
                    ...
                    train_df_task = df[df['split'] == 'test'].copy()
                    val_df_task = df[df['split'] == 'test'].copy()
                    test_df_train = df[df['split'] == 'test'].copy()
